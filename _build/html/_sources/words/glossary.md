...
$\forall$
    *For all ...*
    For example, $\forall x \in \mathbb{N}, x < x + 1$ just says every $x$ is less than $x + 1$ (which is true)

$\exists$
    *There exists...*
    For example, $\exists x \in \mathbb{N}, x = x + 1$ just says there is some number $x$ such that $x = x + 1$ (which is false)
...
Set
    Just a bunch of stuff written between $\{$ and $\}$. See [this section](sections:sets) for more info.

Function
    Mapping between sets, written $f: A \to B$. See [this section](sections:sets:functions) for more info.
